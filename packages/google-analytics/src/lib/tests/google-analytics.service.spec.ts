...
import { provideGoogleAnalytic } from '../providers';
...
describe('GoogleAnalyticsService', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [provideGoogleAnalytic({ trackingCode: tracking })],
    });
  });
  ...
  describe('gtag(`event`, `exception`)', () => {
    ...
    it('should send `description` attribute on gtag command', () => {
      ...
      expect(spyOnGtag).toHaveBeenCalledWith('event', 'exception', { description });
    });
    ...
    it('should send `fatal` attribute on gtag command', () => {
      ...
      expect(spyOnGtag).toHaveBeenCalledWith('event', 'exception', { fatal });
    });
  });
